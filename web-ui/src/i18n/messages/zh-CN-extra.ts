const zhCNExtra = {
  tasks: {
    ...
    form: {
      ...
      excludeKeywordRules: '忽略关键词',
      excludeKeywordRulesHint: '命中任一忽略关键词时，该商品直接判定为不推荐。每行一个关键词，或使用逗号分隔；纯英数字关键词按完整词匹配。',
      excludeKeywordRulesPlaceholder: '示例：维修\n翻新\n仅展示',
      priceRange: '价格范围',
      minPrice: '最低价',
      maxPrice: '最高价',
      maxPages: '搜索页数',
    ...
    table: {
      ...
      excludeKeywordStrategies: '{count} 个忽略词',
      refreshCriteria: '重刷',
      start: '启动',
      stop: '停止',
      stopping: '停止中',
  ...
  notifyPanel: {
    ...
    supportedVariables: '支持变量：title=完整商品标题 / notification_title=短标题 / keyword / content / price / reason / desktop_link / mobile_link / image_url=商品主图 / image_urls=多图链接 / image_gallery=多图 HTML / image_carousel=大图横滑 HTML / xianyu_card=闲鱼风 HTML 卡片',
    globalBehavior: '全局行为',
    ...
    secretPlaceholder: '输入配置值',
    secretKeepPlaceholder: '输入 Token',
    notConfigured: '尚未配置。',
    ntfy: {
      description: '适合轻量推送，URL 非敏感，可直接回显和修改。',
    },
    bark: {
      description: 'URL 含设备 key，当前管理员页面会直接显示。',
      configuredHint: '当前 URL 已加载，可直接修改。',
    },
    gotify: {
    ...
    wecom: {
      title: '企业微信机器人',
      description: 'Bot URL 含 key，当前管理员页面会直接显示，可直接修改或清空。',
      urlLabel: '企业微信 Bot URL',
      configuredHint: '当前机器人地址已加载。',
    },
    telegram: {
      description: 'Bot Token 会在密码框中加载，Chat ID 与反代地址可直接查看和修改。',
      chatIdPlaceholder: '例如：123456789',
      apiBaseUrl: 'API / 反代地址',
    },
    webhook: {
      title: '通用 Webhook',
      description: '支持 JSON 模板变量；URL、Headers 和 Body 会直接回显，便于检查当前配置。',
      urlLabel: 'Webhook URL',
      headersLabel: 'Webhook Headers (JSON)',
      headersPlaceholder: '例如：',
